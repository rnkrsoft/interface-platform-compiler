...
public class JavaInterfacePlatformCompiler implements InterfacePlatformCompiler {
    ...
    void generateServiceClass(CompileContext context, ServiceInfo serviceInfo) throws FileNotFoundException {
        ...
        buf.putUTF8("import ", "com.rnkrsoft.platform.client.async.AsyncTask", ";", "\n");
        buf.putUTF8("\n");
        ...
        for (InterfaceInfo interfaceInfo : serviceInfo.getInterfaces()) {
            ...
            buf.putUTF8("\n");
            buf.putUTF8(context.indent_n(), "@ApidocInterface(value = \"", interfaceInfo.getDesc(), "\", name = \"", interfaceInfo.getName(), "\", version = \"", interfaceInfo.getVersion(), "\", usage = \"", interfaceInfo.getUsage(), "\")\n");
            buf.putUTF8(context.indent_n(), interfaceInfo.getResponseClass().getSimpleName(), " ", interfaceInfo.getMethodName(), "(", interfaceInfo.getRequestClass().getSimpleName(), " request);\n");
            buf.putUTF8("\n");
        }
    ...
    void generateValueObjectClass(CompileContext context, ByteBuf buf, BeanElementInfo elementInfo) throws FileNotFoundException {
        buf.putUTF8("\n");
        buf.putUTF8(context.indent_n(), "public static class ", elementInfo.getJavaClass().getSimpleName(), " implements Serializable", " {\n");
        context.increaseDeep();
    ...
    void generateFormObjectClass(CompileContext context, ByteBuf buf, FormElementInfo elementInfo) throws FileNotFoundException {
        buf.putUTF8("\n");
        buf.putUTF8(context.indent_n(), "public static class ", elementInfo.getBeanClass().getSimpleName(), " implements Serializable", " {\n");
        context.increaseDeep();
        for (ElementInfo column : elementInfo.getElements()) {
            if (column.isValue()) {
                ValueElementInfo valueElementInfo = column.as(ValueElementInfo.class);
    ...
    void generateResponseClass(CompileContext context, InterfaceInfo interfaceInfo) throws FileNotFoundException {
        ...
        buf.putUTF8("public class ", interfaceInfo.getResponseClass().getSimpleName(), (interfaceInfo.isPageable() ? " extends " + AbstractResponsePage.class.getSimpleName() + "<" + interfaceInfo.getResponseClass().getSimpleName() + "." + recordsJavaClass.getSimpleName() + ">" : " extends AbstractResponse"), " {\n");
        context.increaseDeep();
